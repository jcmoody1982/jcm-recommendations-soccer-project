...
public final class ElitePicksSelector {
    ...
    /**
     * Goal lines Elite will not carry, screened on the market string because more than one
     * Elite-eligible type can emit them (OVER_GOALS, UNDER_GOALS, TOP_VS_BOTTOM, VALUE_BET).
     *
     * <p>Over 0.5: half-goals score too high and crowd out everything else. Over 1.5 stays eligible
     * but is capped separately.
     *
     ...
     * construction. Over 2.5 is the highest over-line we trust.
     *
     * <p>Under 1.5: the same construction — the engine only labels Under 1.5 above STRONG, so every
     * pick is Elite-eligible — on a line that lands in about 20% of matches. Under 2.5 stays eligible.
     */
    private static final List<String> EXCLUDED_MARKET_LINES = List.of("over 0.5", "over 3.5", "under 1.5");
